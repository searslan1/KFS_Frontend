...
export default function CampaignsPage() {
  ...
  const filteredCampaigns = allCampaigns
    .filter(
      (campaign) =>
        (selectedSectors.length === 0 || selectedSectors.includes(campaign.sector)) &&
        ...
        (fundingStatus === "all" || campaign.campaign_status === fundingStatus) &&
        (fundingProgress === "all" ||
          (fundingProgress === "0-25" && campaign.currentAmount / campaign.goalAmount <= 0.25) ||
          ...
          (fundingProgress === "76-100" && campaign.currentAmount / campaign.goalAmount > 0.75)) &&
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4">
            <div className="bg-white rounded-lg shadow-md p-4 sticky top-24">
              ...
              <Accordion type="multiple" defaultValue={["sector", "category", "funding-range"]} className="w-full">
                <AccordionItem value="sector">
                  <AccordionTrigger>Sektör</AccordionTrigger>
                  <AccordionContent>
                    {["Teknoloji", "Sağlık", "Eğitim", "Finans", "Enerji"].map((sector) => (
                ...
                <AccordionItem value="category">
                  ...
                  <AccordionContent>
                    {["Yazılım", "Biyoteknoloji", "Yenilenebilir Enerji", "E-ticaret", "Yapay Zeka"].map((category) => (
          ...
          <div className="lg:w-3/4">
            ...
            <div className="bg-white rounded-lg shadow-md p-4 mb-6 flex flex-wrap items-center gap-4 sticky top-24 z-10">
              ...
              <Select onValueChange={(value) => setSortOption(value as SortOption)} defaultValue="fundingAsc">
                <SelectTrigger className="w-[180px]">
                  <SelectValue placeholder="Sıralama" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="fundingAsc">Fonlama: Azdan Çoğa</SelectItem>
                  <SelectItem value="fundingDesc">Fonlama: Çoktan Aza</SelectItem>
                  <SelectItem value="dateAsc">Tarih: Eskiden Yeniye</SelectItem>
                  <SelectItem value="dateDesc">Tarih: Yeniden Eskiye</SelectItem>
                </SelectContent>
              </Select>

              <Select onValueChange={(value) => setFundingStatus(value as FundingStatus)} defaultValue="all">
                <SelectTrigger className="w-[180px]">
              ...
              <Select onValueChange={(value) => setFundingProgress(value)} defaultValue="all">
                <SelectTrigger className="w-[180px]">
              ...
              <div className="lg:hidden bg-white rounded-lg shadow-md p-4 mb-6">
                ...
                <Accordion type="multiple" defaultValue={["sector", "category", "funding-range"]} className="w-full">
                    ...
                    <AccordionContent>
                      {["Teknoloji", "Sağlık", "Eğitim", "Finans", "Enerji"].map((sector) => (
                        <div key={sector} className="flex items-center mb-2">
                          ...
                          />
                          <label htmlFor={`mobile-sector-${sector}`} className="ml-2 text-sm">
                            {sector}
                          </label>
                        </div>
                      ))}
                  ...
                  <AccordionItem value="category">
                    <AccordionTrigger>Kategori</AccordionTrigger>
                    <AccordionContent>
                      {["Yazılım", "Biyoteknoloji", "Yenilenebilir Enerji", "E-ticaret", "Yapay Zeka"].map(
                        (category) => (
                          <div key={category} className="flex items-center mb-2">
                            <Checkbox
                            ...
                            />
                            <label htmlFor={`mobile-category-${category}`} className="ml-2 text-sm">
                              {category}
                            </label>
                          </div>
                        ),
            ...
            {filteredCampaigns.length === 0 ? (
              <p className="text-center text-lg text-gray-600">Aradığınız kriterlere uygun kampanya bulunamadı.</p>
            ) : (
