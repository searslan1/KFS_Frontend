...
import { ProductSection } from "@/components/campaign-sections/ProductSection"
...
export default function CampaignCreatePage() {
  ...
  const handleSave = async () => {
    ...
    const updatedSections = sections.map((s) => (s.id === activeSection ? { ...s, isCompleted: true } : s))
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-[#4DB05F]/5 to-white">
      <main className="flex-grow p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-200">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Kampanya Oluştur</h1>
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    section.isCompleted ? "bg-[#4DB05F]/5" : "",
                  ...
                  <div className="flex flex-col items-center justify-center">
                    <div
                      className={cn(
                        ...
                        activeSection === section.id ? "bg-[#4DB05F] text-white" : "bg-gray-100 text-gray-600",
                      )}
                    >
                      <Icon className="h-3 w-3 sm:h-4 sm:w-4" />
                    </div>
                    <h3 className="text-xs font-medium text-center">{section.title}</h3>
                  </div>
                  {activeSection === section.id && <div className="bg-[#4DB05F] h-0.5 w-full mt-1" />}
                </motion.div>
              )
            })}
          </div>
            ...
            >
              <div className="flex justify-between items-center mb-4 sm:mb-6">
                ...
                <div className="flex gap-2 sm:gap-3">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="outline"
                          size="icon"
                          onClick={() => setActiveSection(prevSection?.id || sections[sections.length - 1].id)}
                  ...
                  <Button
                    className="bg-[#4DB05F] hover:bg-[#4DB05F]/90 text-white text-sm sm:text-base px-3 sm:px-4"
                    onClick={handleSave}
                    disabled={isSaving}
                  >
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="outline"
                          size="icon"
                          onClick={() => setActiveSection(nextSection?.id || sections[0].id)}
              ...
              <div className="min-h-[300px] sm:min-h-[400px] space-y-8">
                ...
                {activeSection === "documents-other" && <OtherDocumentsSection />}
              </div>
            </motion.div>
          </AnimatePresence>
          ...
          <div className="mt-6 flex justify-between">
            <Button
              variant="outline"
              className="text-[#4DB05F] border-[#4DB05F] hover:bg-[#4DB05F]/10 text-sm sm:text-base"
              onClick={() => {}}
            >
              <Eye className="h-4 w-4 mr-2" />
              Önizleme
            </Button>
            <Button className="bg-[#4DB05F] hover:bg-[#4DB05F]/90 text-white text-sm sm:text-base" onClick={() => {}}>
