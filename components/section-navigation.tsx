          ...
          <li key={section.id} className="relative">
            <button
              ...
              className={`text-xs font-medium transition-colors duration-200 ${
                activeSection === section.id ? "text-[#4DB05F]" : "text-gray-400 hover:text-gray-600"
              }`}
            >
              {section.name}
            </button>
            {activeSection === section.id && (
              <motion.div
                className="absolute right-0 top-1/2 w-8 h-0.5 bg-[#4DB05F] transform -translate-y-1/2"
